...
$_ASSET_ROOT = 'asset/img/';

$_w = isset($_REQUEST['w']) ? $_REQUEST['w'] : "";
$_h = isset($_REQUEST['h']) ? $_REQUEST['h'] : "";
$_zc = isset($_REQUEST['zc']) ? $_REQUEST['zc'] : "";
$_src = isset($_REQUEST['src']) ? $_REQUEST['src'] : "";
$_q = isset($_REQUEST['q']) ? $_REQUEST['q'] : "";

if(empty($_src)){
	die('No source specied');
}

// clean params before use
$src		= preg_replace( "/^(\.+(\/|))+/", "", $_src);
$src		= preg_replace( '/^(s?f|ht)tps?:\/\/[^\/]+/i', '', $src );
$new_width	= preg_replace( "/[^0-9]/", "", $_w  );
$new_height = preg_replace( "/[^0-9]/", "", $_h );
$zoom_crop	= preg_replace( "/[^0-9]/", "", $_zc );
$quality	= preg_replace( "/[^0-9]/", "", $_q );
...
$src2 = $_SERVER['DOCUMENT_ROOT'] . '/' . $_ASSET_ROOT. str_replace('asset/','',$src);
$src3 = $_ASSET_ROOT. str_replace('asset/','',$src);
$file = "";
if(file_exists($src1)){
	$file = $src1;
}
...
$image = open_image($mime_type, $file);
if ($image === false) {
...
function show_cache_file ( $cache_dir ) {
	...
	$file_full_path = $cache_dir . '/' . $cache_file;
    if( file_exists( $file_full_path ) ) {

    	// check for updates
		$gmdate_mod	= gmdate('D, d M Y H:i:s', filemtime( $file_full_path ) ) . " GMT";
